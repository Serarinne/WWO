...
<head>
    ...
    <title>Blog Posts in {{ $category->name ?? 'Review' }} | {{ env('APP_NAME') }}</title>

    <link rel="canonical" href="{{ url()->current() }}" />
    <meta name="description" content="Find the latest news, updates, and articles about {{ $category->name ?? 'Review' }} wallpapers on the {{ env('APP_NAME') }} blog. Explore posts in this category.">
    <meta name="keywords" content="waifuwall, {{ $category->name ?? 'review' }}, anime blog category, anime wallpaper news">

    <meta name="robots" content="index, follow, max-image-preview:large">

    <meta property="og:locale" content="en_US" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="Blog Posts in {{ $category->name ?? 'Review' }}" />
    <meta property="og:description" content="Browse articles in the {{ $category->name ?? 'Review' }} category." />
    <meta property="og:url" content="{{ url()->current() }}" />
    <meta property="og:site_name" content="{{ env('APP_NAME') }}" />

    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="Blog Posts in {{ $category->name ?? 'Review' }}" />
...
<body class="bg-[#0f172a] text-gray-200 font-sans min-h-screen flex flex-col selection:bg-indigo-500 selection:text-white">
    ...
    <main class="flex-grow">
        ...
        <div class="relative bg-gray-900 border-b border-gray-800 py-12 md:py-16">
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div class="flex flex-col items-center text-center">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 mb-4 uppercase tracking-wider">
                        Category Archive
                    </span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                @empty
                    <div class="col-span-full py-20 text-center bg-gray-800/30 rounded-2xl border border-gray-700/50 border-dashed">
                        ...
                        <a href="{{ route('posts.index') }}" class="mt-6 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 transition-colors">
                            Back to Blog
                        </a>
                    </div>
                @endforelse
