...
<head>
    ...
    @else
        <meta property="og:image" content="{{ $wallpaper->thumbnail['jpg'] }}" />
    ...
    <style>
        ...
        .ts-control { background-color: #374151 !important; border-color: #4b5563 !important; color: #fff; }
        .ts-dropdown { background-color: #1f2937 !important; border-color: #4b5563 !important; color: #fff; }
        .ts-dropdown .option:hover, .ts-dropdown .active { background-color: #374151 !important; color: #fff; }
        .ts-control input { color: #fff !important; }
        .ts-wrapper.multi .ts-control > div { background: #4b5563; color: #fff; border-radius: 4px; }
    </style>
    ...
    "itemListElement": [{
        ...
        "@@type": "ListItem", 
        "position": 2, 
        "name": "{{ $wallpaper->characters[0]->series->first()->name }}",
        "item": "{{ route('series.view', ['series' => $wallpaper->characters[0]->series->first()]) }}"  
    },{
        "@@type": "ListItem", 
        "position": 3, 
        "name": "{{ $wallpaper->characters[0]->name }}",
        "item": "{{ route('characters.view', ['series' => $wallpaper->characters[0]->series->first(), 'character' => $wallpaper->characters[0]]) }}"  
    },{
        "@@type": "ListItem", 
        "position": 4, 
    ...
    @else
        <script type="application/ld+json">
        {
            ...
            "creditText": "{{ $wallpaper->artists->first()->name ?? 'Unknown' }}",
            "creator": {
                "@@type": "Person",
                "name": "{{ $wallpaper->artists->first()->name ?? 'Unknown' }}"
            },
            "copyrightNotice": "{{ $wallpaper->artists->first()->name ?? 'Unknown' }}",
            @if($wallpaper->source != null || $wallpaper->source != "")
            "sameAs": "{{ $wallpaper->source }}",
            @endif
...
<body class="bg-gray-900 text-gray-200 font-sans">
    ...
    <main class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="max-w-7xl mx-auto mb-6">
            <h1 class="text-3xl md:text-4xl font-bold text-white">{{ $wallpaper->seo_title }}</h1>
        </div>

        <div class="w-full mx-auto rounded-lg overflow-hidden shadow-2xl bg-gray-800">
            @if ($wallpaper->is_video)
                <video controls controlslist="nodownload" class="w-full rounded-lg max-h-[80vh]" preload="metadata">
        ...
        @if(!empty($wallpaper->image_description))
            <section class="max-w-7xl mx-auto mt-6">
                <div class="bg-gray-800/50 rounded-lg p-5 md:p-6">
        ...
        <div class="max-w-7xl mx-auto mt-6 flex flex-col sm:flex-row gap-4" x-data="{ 
            ...
            <form action="{{ route('wallpapers.download', $wallpaper->id) }}" method="POST" class="flex-grow">
                ...
                <button type="submit"
                    class="w-full flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg text-lg transition-colors">
            ...
            <div class="flex gap-4">
                ...
                <button 
                    ...
                    :disabled="isLoading"
                    class="flex-grow flex items-center justify-center gap-2 font-bold py-3 px-6 rounded-lg transition-colors" 
                    :class="isFavorited ? 'bg-pink-600 text-white' : 'bg-gray-700 hover:bg-gray-600'">
                ...
                @endauth
                @guest
                <a href="{{ route('login.index') }}" class="flex-grow flex items-center justify-center gap-2 font-bold py-3 px-6 rounded-lg transition-colors bg-gray-700 hover:bg-gray-600">
        ...
        <div class="max-w-7xl mx-auto mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-8">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <a href="{{ route('tags.view', $tag) }}" class="bg-gray-700 hover:bg-gray-600 text-sm font-medium px-4 py-1.5 rounded-full transition-colors">{{ $tag->name }}</a>
                        @endforeach
                    </div>
                </div>
                @endif
                    ...
                    <div class="space-y-3">
                        @foreach($groupedCharacters as $character)
                            <div class="character-group">
                                <a href="{{ route('characters.view', ['series' => $character->series->first()->slug, 'character' => $character->slug]) }}" 
                                   class="relative z-10 flex items-center gap-4 bg-gray-800 hover:bg-gray-700 p-3 rounded-lg transition-colors {{ isset($character->sub_versions) && $character->sub_versions->count() > 0 ? 'border-l-4 border-cyan-500' : '' }}">
                                    <picture>
                                ...
                                @if(isset($character->sub_versions))
                                    @foreach($character->sub_versions as $child)
                                        <div class="relative">
                                            <div class="absolute -top-4 left-6 w-6 h-10 border-b-2 border-l-2 border-gray-600 rounded-bl-lg pointer-events-none"></div>
                                            <a href="{{ route('characters.view', ['series' => $child->series->first()->slug, 'character' => $child->slug]) }}" 
                                               class="ml-10 flex items-center gap-4 bg-gray-800/80 hover:bg-gray-700 p-2.5 rounded-lg transition-colors group mt-2">
                                                <picture>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('artists.view', ['slug' => $artist]) }}" class="flex items-center gap-4 bg-gray-800 hover:bg-gray-700 p-3 rounded-lg transition-colors">
                            <picture>
            ...
            <aside class="lg:col-span-1 space-y-8">
                <div class="bg-gray-800/50 p-5 rounded-lg">
                    <div class="text-lg font-semibold text-white mb-4">Information</div>
                    <dl class="text-sm space-y-4">
                        ...
                        <div class="flex justify-between items-start">
                            ...
                            <dd><a href="{{ $wallpaper->real_source }}" target="_blank" rel="noopener nofollow" class="text-blue-400 hover:underline break-all text-right">Link</a></dd>
                        </div>
                        @endif
                    </dl>
                </div>
