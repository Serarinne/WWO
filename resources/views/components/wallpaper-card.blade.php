...
<div class="mb-4 break-inside-avoid group relative rounded-xl overflow-hidden bg-gray-800 shadow-md hover:shadow-2xl hover:shadow-cyan-500/10 transition-all duration-300 hover:-translate-y-1">
    <a href="{{ route('wallpapers.view', $wallpaper) }}" class="block">
        ...
        <picture>
            ...
            <img src="{{ $wallpaper->thumbnail['webp'] }}" 
                    ...
                    width="300" 
                    height="{{ round((($wallpaper->height/$wallpaper->width)*300),0) }}" 
                    loading="lazy" 
        ...
        <div class="absolute inset-0 bg-gradient-to-t from-gray-900 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 z-10">
            ...
            <div class="flex items-center justify-between mt-1">
                ...
                <span class="text-cyan-400 text-xs font-semibold">View &rarr;</span>
            </div>
        </div>

        @if ($wallpaper->is_video)
            <div class="absolute top-2 right-2 z-20 bg-black/60 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center border border-white/10 shadow-sm pointer-events-none">
